    ...
    return (
        <>
            <div key={props.project.index} className={props.project.index % 2 === 0 ? "timeline-grid" : "timeline-grid timeline-grid-right"}>
                ...
                <div
                     className={`timeline-modal transform scale-0 transition-transform duration-300 ${isOpen ? "scale-100" : ""}`}>
                    <div className="timeline-modal-card">
                        <p className="timeline-modal-image">
                            <img className="max-h-full py-5" src={props.project.image}
                                 alt={props.project.title}/>
                        </p>
                        <button onClick={() => setOpen(isOpen => !isOpen)}
                                className="timeline-modal-close">
                            <svg className="h-6 w-6 dark:text-gray-400"
                                 fill="none"
                                 viewBox="0 0 24 24"
                                 stroke="currentColor">
